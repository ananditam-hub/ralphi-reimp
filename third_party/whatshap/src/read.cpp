...
Read::Read(const std::string& name, int mapq, int source_id, int sample_id, int reference_start, const std::string& BX_tag, const std::string& strand) :
name(name), mapqs(1, mapq), source_id(source_id), sample_id(sample_id), reference_start(reference_start),
BX_tag(BX_tag), strand(strand) {
	this->id = -1;
}
...
void Read::removeVariant(int position) {
    int index = -1;
    for (size_t i=0; i < variants.size(); ++i) {
		if (variants[i].position == position) {
		    index = i;
		    break;
		}
	}
	//std::cout << "Removing variant in read " << name << " at index " << index << " at position " << position << "\n";
...
const std::string& Read::getStrand() const {
	return strand;
}
