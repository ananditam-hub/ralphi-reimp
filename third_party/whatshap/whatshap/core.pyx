...
cdef class Read:
	...
	str BX_tag = None, str strand = None):
		cdef string _name = b''
		cdef string _BX_tag = b''
		cdef string _strand = b''
		if name is None:
			self.thisptr = NULL
			self.ownsptr = False
		else:
			...
			if strand is not '' and strand is not None:
				_strand = strand.encode('UTF-8')
			self.thisptr = new cpp.Read(_name, mapq, source_id, sample_id, reference_start, _BX_tag, _strand)
			self.ownsptr = True
	...
	def __repr__(self):
		assert self.thisptr != NULL
		return 'Read(name={!r}, mapq={}, source_id={}, sample_id={}, reference_start={},  BX_tag={}, variants={}, strand={})'.format(
			self.name, self.mapqs, self.source_id, self.sample_id, self.reference_start, self.BX_tag, list(self), self.strand)
	...
	property strand:
		def __get__(self):
			assert self.thisptr != NULL
			return self.thisptr.getStrand().decode('utf-8')
	...
	def __getstate__(self):
		...
		return (mapqs, self.name, self.source_id, self.sample_id, self.reference_start, self.BX_tag, self.strand, variants)
	
	def __setstate__(self, state):
		mapqs, name, source_id, sample_id, reference_start, BX_tag, strand, variants = state
		...
		if name is None:
			...
			if BX_tag is not b'' and BX_tag is not None:
				_BX_tag = BX_tag.encode('UTF-8')
			if strand is not b'' and strand is not None:
				_strand = strand.encode('UTF-8')
			self.thisptr = new cpp.Read(_name, mapqs[0] if len(mapqs) > 0 else 0, source_id, sample_id, reference_start, _BX_tag, _strand)
			self.ownsptr = True
	...
	def remove_variant(self, int position):
		assert self.thisptr != NULL
		self.thisptr.removeVariant(position)
