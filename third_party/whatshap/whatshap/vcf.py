...
class BiallelicVcfVariant(VcfVariant):
    ...
    def __init__(self, position: int, reference_allele: str, alternative_allele: str, index: int):
        """
    ...
    def __repr__(self):
        return "BiallelicVcfVariant({}, {!r}, {!r}, {})".format(
            self.position, self.reference_allele, self.alternative_allele, self.index
        )
    ...
    def normalized(self) -> "BiallelicVcfVariant":
        ...
        return BiallelicVcfVariant(pos, ref, alt, self.index)
        ...
        for idx, record in enumerate(records):
            ...
            if len(alts) == 1:
                variant: VcfVariant = BiallelicVcfVariant(
                    position=pos, reference_allele=ref, alternative_allele=alts[0], index=idx
                )
            else:
